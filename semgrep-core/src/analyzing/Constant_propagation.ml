      ...
      V.kdef = (fun (k, v) x ->
        ...
        | { name = EN (Id (id,
          ...
          VarDef ({ vinit = Some e; _ }) ->
             ...
             * re-visited but now interpreted as an assignment, and we would be
             * wrongly incrementing `stat.lvalue` a second time. Instead, we just
             * need to visit the defining expression. *)
            v (E e)
        | _ -> k x
      );
